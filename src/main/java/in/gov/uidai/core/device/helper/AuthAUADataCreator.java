...
public class AuthAUADataCreator {
	...
	public AuthDataFromDeviceToAUA prepareAUAData(String uid, String terminalId, Meta meta, Object pid, DataType dataType) {
		...
		try {
			...
			/*if (DataType.P == dataType) {
				rawPid = ((in.gov.uidai.core.model.protobuf.auth.Auth.Pid) pid).toByteArray();
				System.out.println("Proto Pid in Hex: " + Hex.encodeHexString(rawPid));
			} else {
				rawPid = createPidXML((Pid) pid).getBytes();
			}*/
			rawPid = createPidXML((Pid) pid).getBytes();
			...
			byte[] demoBytes;
			/*if (DataType.P == dataType) {
				demoBytes = ((in.gov.uidai.core.model.protobuf.auth.Auth.Pid) pid).getDemo().toByteArray();
			} else {
				demoBytes = getDemoXML((Pid) pid).getBytes();
			}*/
			demoBytes = getDemoXML((Pid) pid).getBytes();
	 ...
	 * used for verifying Demo XML hash received in core responses.
	 * 
	 * @param pid
	 *            Pid instance
